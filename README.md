...
По сути, `txt.wwew.tech` — это промежуточный слой между вашими исходниками и LLM: меньше шума, больше полезного контекста.

## 🧱 Возможности интерфейса

### Левая панель
- Бренд и запуск нового чата
- История из `localStorage`
- Сворачивание панели
- Переключение темы
- Полезные ссылки

Левая панель отвечает за навигацию и сессии: удобно возвращаться к предыдущим наборам данных и быстро стартовать новый контекст.
...
Центральная зона — основное рабочее пространство, где происходит сборка и просмотр содержимого перед экспортом.
...
Правая панель управляет приватностью и качеством итогового контекста: можно быстро отсечь служебные директории и нерелевантные типы файлов.
